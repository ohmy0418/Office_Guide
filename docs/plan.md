...
- 결과 결합 기본값은 RRF를 사용한다. 가중합 방식은 품질 비교·실험 시 설정으로 전환해 테스트한다.
...
| 답변 생성 모델 | OpenAI | 기본 답변 생성 모델검색 결과를 바탕으로 답변 생성 |
...
2. **결과 결합**: **기본값은 RRF**. **가중합** 결합은 동일 데이터로 비교 실험·튜닝 시 사용한다.
...
5. **동의어/사전**: 운영 주체·운영 UI/API는 **v1 범위에서 제외**(보류). 제품 기능으로는 넣지 않는다. 질문 정규화에 필요한 **코드 내 고정 치환**만 허용한다.
