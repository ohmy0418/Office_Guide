...
4. 혼합 조회 응답 스키마에 `sections`, `time_basis`, `partial_answer`, `fallback_reason` 등을 포함할지 여부
5. route_type / status / error_code enum 표준안 확정
...
- route_type / status / error_code enum 초안 작성
- API 응답 공통 포맷 초안 작성

### 공통 enum 정의
- route_type / status / document_status / failure_reason enum 표준 확정
...
3. route_type / status / error_code enum 표준안 확정
4. 혼합 조회 응답 스키마 확정
